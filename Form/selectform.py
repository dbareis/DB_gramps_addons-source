...
class SelectForm(object):
    ...
    def _hover_expand_toggled(self, cbtn):
        self.hover_expand = cbtn.get_active()
        self.tree.set_hover_expand(self.hover_expand)
        _LOG.debug('hover expand cb toggled, ON=' +
                   str(self.hover_expand))
    ...
    def _create_dialog(self):
        ...
        cbtn = Gtk.CheckButton(_('Hover E_xpand'), use_underline=True)
        cbtn.set_tooltip_text(_('If hover expand is turned on the' +
                                ' tree nodes will expand and' +
                                ' contract when you hover the mouse' +
                                ' over them)'))
        cbtn.set_active(self.hover_expand)
        cbtn.connect('toggled', self._hover_expand_toggled)
        hbox = Gtk.Box()
        hbox.set_spacing(3)
        hbox.pack_start(label, True, True, 5)
        hbox.pack_end(cbtn, False, False, 5)
        top.vbox.pack_start(hbox, 0, 0, 5)
        ...
        compact_button = top.add_button(_('_Pack'),
                                        self.RESPONSE_COLLAPSE_TREE)
        top.add_button(_('_Cancel'), Gtk.ResponseType.CANCEL)
        ok_button = top.add_button(_('_OK'), Gtk.ResponseType.OK)
        top.show_all()
        ...
        st3 = _('Matches occur if the text (in the tree or a source)' +
                ' CONTAINS the case insensitive search text you typed.')
        ...
        # Want Focus on tree (for start typing "search" to work)
    ...
    def run(self):
        ...
        try:
            ...
            while True:
                ...
                CONFIG.set('selectform.hover-expand', self.hover_expand)
                CONFIG.save()
